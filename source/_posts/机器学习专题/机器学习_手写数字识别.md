...
tags:
  - scikit-learn
  - 手写数字识别
date: 2016-06-20 19:49:11
---

MINIST 数据文件（http://yann.lecun.com/exdb/mnist/）
<!--more-->
数据读取参考教程（http://blog.csdn.net/dream_angel_z/article/details/50739817）

# 读取数据保存成图片

读取MINIST原始数据格式的工具函数，来源：http://blog.csdn.net/dream_angel_z/article/details/50739817
```python
# -*- coding: utf-8 -*-
...
class DataUtils(object):
    ...
    def outImg(self, arrX, arrY):
        ...
        for i in range(m):
            ...
            plt.imsave(self._outpath + "/" + outfile, img, cmap=plt.cm.gray)
```
...
import numpy as np
reload(sys)
sys.setdefaultencoding('utf8') #主要是解决编码不一致问题，没有问题这句可以不加

trainfile_X = 'D:/train-images.idx3-ubyte'
trainfile_y = 'D:/train-labels.idx1-ubyte'
testfile_X = 'D:/t10k-images.idx3-ubyte'
testfile_y = 'D:/t10k-labels.idx1-ubyte'
...
#以下内容是将图像保存到本地文件中
path_trainset = "D:\imgs_train"
path_testset = "D:\imgs_test"
if not os.path.exists(path_trainset):
    os.mkdir(path_trainset)
if not os.path.exists(path_testset):
    os.mkdir(path_testset)
...
DataUtils(outpath=path_testset).outImg(test_X, test_y)
```
程序执行完成后会在本地保存好60000张训练图片和10000张测试图片
![train_image](train_image.png)
![test_image](test_image.png)

# 加载数据
下面以这些图片作为原始数据，开始手写数字识别。
