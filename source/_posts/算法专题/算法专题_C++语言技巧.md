...
#include <deque>
deque<int> d;
d.push_back(1); // 队尾插入元素
d.push_front(2); // 队首插入元素
d.pop_front(); // 队首弹出元素
d.pop_back(); // 队尾弹出元素
d.front(); //获得队首元素
d.back(); //获得队尾元素
...
// 创建链表
fl.insert_after(fl.before_begin(),10,1); // 10 个 1
fl.insert_after(fl.before_begin(),v.begin(),v.end()); // 1->2->3->4->5
// 获得某个节点
auto iter = fl.before_begin(); // 头指针之前的节点，类似哨兵的作用，可以使用它在链表的头部插入元素
auto head = fl.begin(); // 指向头结点的指针
auto head_value = *head; //第一个节点的值
iter++; // 移动一步
if(iter == head) cout<<"true"<<endl; // 判断是否指向相同的元素
auto front = fl.front(); // 第一个元素
auto end = fl.end(); // 最后一个元素的下一个位置
fl.reverse(); // 翻转链表
// 删除节点
fl.erase_after(fl.before_begin()); // 删除第一个节点
fl.erase_after(fl.begin(),fl.end()); //删除第一个节点之后的元素直到链表的结尾
// 此外，还提供了合并，排序，判空，清空等方法
...
priority_queue 可实现最大堆，最小堆和优先队列等数据结构。
 
### 红黑树

set,map, multiset, multimap 都是基于红黑树实现的。这里面的元素是有顺序的。
